...
function WvDialConf(file) {
  ...
  this.file = file || '/etc/WvDialConf.conf';
}
...
WvDialConf.prototype.save = function (data, cb) {
  ...
  fs.writeFile(this.file, ini.encode(data), 'utf-8', function (err) {
    if (err) return cb(err);
    cb();
  });
  ...
  this.load()
    .then(function (config) {
      if (config[section] && config[section][key]) {
        return cb(null, config[section][key]);
      }
      cb(null, config[key]);
    })
...
WvDialConf.prototype.set = function (/*section, key, value, cb*/) {
  ...
  if (!props) {
    cb(new Error('No key and value provided'));
    return cb.promise;
  }
...
WvDialConf.prototype.setProvider = function (provider, cb) {
  cb = cb || proback();
  this.set(_.pick(provider, ['phone', 'username', 'password']), cb);
  return cb.promise;
};
